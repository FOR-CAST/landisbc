...
Fetches the provincial fuel-type polygons over a study area.
`FUEL_TYPE_CD` is returned as a factor (the categorical field that
`rasterize()` / `freq()` summarise by label). The WFS query is done in
BC Albers (EPSG:3005) – the WFS cannot interpret a custom simulation CRS
– and the result is reprojected back to the study-area CRS.

## Usage

``` r
get_fuel_types(study_area)
```
...
- study_area:
  ...
  `sf` or `SpatVector` study-area polygon (the buffered simulation
  area).

## Value

An `sf` of provincial fuel-type polygons with a factor `FUEL_TYPE_CD`
column, clipped to the study area.
